...
if (!class_exists('TCPDF', false)) {
	 ...
	 */ 
	define('PDF_PRODUCER','TCPDF 4.1.004 (http://www.tcpdf.org)');
	...
	class TCPDF {
		...
		*/
		public function setPageOrientation($orientation, $autopagebreak='', $bottommargin='') {
			...
			$this->pagedim[$this->page] = array('w' => $this->wPt, 'h' => $this->hPt, 'wk' => $this->w, 'hk' => $this->h, 'tm' => $this->tMargin, 'bm' => $bottommargin, 'lm' => $this->lMargin, 'rm' => $this->rMargin, 'pb' => $autopagebreak, 'or' => $this->CurOrientation);
		}
		...
		*/
		...
		public function Header() {
			...
			//ESTO FUE MODIFICADO POR ROBERTO $this->Cell(0, 0, '', 'T', 0, 'C');
		}
	...
	Putting 1 is equivalent to putting 0 and calling Ln() just after. Default value: 0.
					...
					if (isset($startlinex)) {
						...
						if ((isset($plalign) AND ((($plalign == "C") OR ($plalign == "J") OR (($plalign == "R") AND (!$this->rtl)) OR (($plalign == "L") AND ($this->rtl))))) OR ($yshift < 0)) {
							...
							if ($plalign == "J") {
								...
								$ns = 0;
								// search spaces
								if (preg_match_all('/\[\(([^\)]*)\)\]/x', $pmid, $string, PREG_PATTERN_ORDER)) {
									...
									$spacewidth = (($w - $linew + (($no - $ns) * $this->GetStringWidth(chr(32)))) / ($ns?$ns:1)) * $this->k;
									$spacewidthu = ($w - $linew + ($no * $this->GetStringWidth(chr(32)))) / ($ns?$ns:1) / $this->FontSize / $this->k;
									$ns = 0;
									reset($string);
									$offset = 0;
									$strcount = 0;
						...
						if (($dom[$key]['value'] == "td") OR ($dom[$key]['value'] == "th")) {
							...
							if (isset($dom[$table_el]['rowspans'])) {
								...
								while ($rsk < $rskmax) {
									...
									if  (($trwsp['startx'] > ($this->x - $cellspacing - $currentcmargin)) AND (($trwsp['startx'] < ($this->x + $cellspacing + $currentcmargin))) AND (($trwsp['starty'] < $this->y) OR ($trwsp['startpage'] < $this->page)) AND ($trwsp['rowspan'] > 0)) {
		...
		protected function openHTMLTagHandler(&$dom, $key, $cell=false) {
			...
			switch($tag['value']) {
				...
				case 'hr': {
					$this->addHTMLVertSpace(1, $cell, '', $firstorlast);
					$this->htmlvspace = 0;
					...
					$this->addHTMLVertSpace(1, $cell, '', !isset($dom[($key + 1)]));
					break;
				}
				case 'a': {
				...
				case 'blockquote': {
					...
					$this->addHTMLVertSpace(2, $cell, '', $firstorlast);
					break;
				}
				case 'br': {
					$this->Ln('', $cell);
				...
				case 'p': {
					/*
						EVITA EL SALTO DE LINEA PARA LA ETIQUETA ANTES DE LA IMPRESION DE SU CONTENIDO SU VALOR POR DEFECTO ES DOS NO UNO
					*/
					$this->addHTMLVertSpace(1, $cell, '', $firstorlast); 					
					break;
				}
				...
				case 'h5': 
				case 'h6': {
					$this->addHTMLVertSpace(1, $cell, ($tag['fontsize'] * 1.5) / $this->k, $firstorlast);
					break;
				}
				default: {
					break;
		...
		protected function closeHTMLTagHandler(&$dom, $key, $cell=false) {
			...
			switch($tag['value']) {
				...
				case 'table': {
					...
					foreach ($table_el['trids'] as $j => $trkey) {
						...
						foreach ($parent['cellpos'] as $k => $cellpos) {
							if (isset($cellpos['rowspanid']) AND ($table_el['rowspans'][($cellpos['rowspanid'])]['endpage'] > $parent['endpage'])) {
								$dom[$trkey]['endy'] = $this->pagedim[$dom[$trkey]['endpage']]['hk']  - $this->pagedim[$dom[$trkey]['endpage']]['bm'];
							}
						}
					}
					// for each row
				...
				case 'p': {
					/*
						EVITA EL SALTO DE LINEA PARA LA ETIQUETA DESPUES DE LA IMPRESION DE SU CONTENIDO EL VALOR DE SALTO POR DEFECTO ES DOS NO CERO
					*/
					//$this->addHTMLVertSpace(1, $cell, '', $firstorlast);
					$this->addHTMLVertSpace(1, $cell, 1, false);
					break;
				}
				case 'dl': {
					$this->listnum--;
				...
				case 'h5': 
				case 'h6': {
					$this->addHTMLVertSpace(1, $cell, ($parent['fontsize'] * 1.5) / $this->k, $firstorlast);
					break;
				}
				default : {
					break;
		 ...
		 * @access protected
		 */
		protected function addHTMLVertSpace($n, $cell=false, $h='', $firstorlast=false) {
			if ($firstorlast) {
				$this->Ln(0, $cell);
				$this->htmlvspace = 0;
				return;
